export class CommentDto {
    constructor(json) {
        ...
        if(json.text.includes('/list') || json.text.includes('/board')) {
            const splitText = json.html_text.split('href="');
            splitText.forEach(item => {
                if(item.startsWith('https://app.asana.com') && (item.includes('/list') || item.includes('/board'))) {

                   const user = item.split('>@')[1].split('</a>')[0];
                   const url = item.split('\" data-asana-gid=')[0];

                    this.text = json.text.split(url).join(`${user}`);
                }
            })
        }
        else {
